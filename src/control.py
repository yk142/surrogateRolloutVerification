...
# M16 (#33): パラメータ誤差でトルク余裕が約5%減った結果、W_CONTROL=0.01 では
# 「何もしない」方がコスト最小になりオラクルMPCすらスイングアップしなくなった
# (M4で最初に踏んだのと同じ、サンプリングMPCのコスト設計の脆弱性)。
# 制御コストの重みを下げて行動する誘因を回復させる。オラクル・サロゲート双方に
# 同一に適用されるため、両者の比較の公平性は保たれる。
W_CONTROL = 0.001
